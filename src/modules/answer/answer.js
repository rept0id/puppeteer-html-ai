import puppeteer from 'puppeteer'; // for local use
import syncHtml from './utils/syncHtml.js';
...
async function syncHtmlAnswer(htmlAns, page, selectorPage) {
    ...
    const browserAns = await puppeteer.launch({ headless: true });
    ...
    await syncHtml(page, selectorPage, pageAns);
...
async function fetchAnswer(html, contextAi, keyAi) {
    try {
      ...
      const reqAi = await fetch("https://api.openai.com/v1/chat/completions", {
          ...
          body: JSON.stringify({
              model: "gpt-3.5-turbo",
              messages: [{role: "user", content: contentReqAi}],
...
export async function answer(contextAi, keyAI, page, selectorPage, errSelectorPage=null) {
  let html;
  let htmlAns;
